...
interface GenericReorderPageProps {
    title: string;
...
export const GenericReorderPage: React.FC<GenericReorderPageProps> = ({
    title,
    fetchEndpoint,
    saveEndpoint,
    backUrl,
    ...
    useEffect(() => {
        const fetchAll = async () => {
            try {
                // We fetch all at once to ensure accurate ranking
                const res = await fetch(`${fetchEndpoint}?limit=1000&summary=true`);
                const json = await res.json();
                if (json.data) {
                    const mapped = json.data.map((p: any) => ({
                        id: p.id,
                        title: p.title,
                        image: p.image || (itemType === 'blog' ? extractFirstImage(p.content) : null), // Handle blog images
                ...
                console.error(e);
                addToast(`Failed to load ${itemType}s`, "error");
            } finally {
                setIsLoading(false);
            }
        };
        fetchAll();
    }, [addToast, fetchEndpoint, itemType]);
    ...
    const handleDragOver = (e: React.DragEvent, index: number) => {
        e.preventDefault();
        const globalIndex = (currentPage - 1) * itemsPerPage + index; // Target
        ...
        setItems(newList);
        setDraggedItemIndex(globalIndex); // Update dragged index to follow the item
    };
    ...
    const handleRankChange = (id: number, newRankStr: string) => {
        ...
        if (isNaN(newRank) || newRank < 1 || newRank > items.length) return;
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-900 pb-20">
            <header className="sticky top-0 z-30 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 px-6 py-4 flex items-center justify-between">
                <div className="flex items-center gap-4">
                    <Link href={backUrl} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full transition-colors">
            ...
            <main className="max-w-3xl mx-auto px-4 py-8 space-y-6">
                <div className="bg-blue-50 dark:bg-blue-900/20 text-blue-800 dark:text-blue-300 px-4 py-3 rounded-xl text-sm flex items-start gap-3 border border-blue-200 dark:border-blue-800/50">
                    ...
                    <p>
                        Drag and drop items to reorder them, or type a number in the "Rank" box to jump to a specific position.
                        <strong> Rank 1</strong> will be the first item displayed.
                    </p>
                </div>
