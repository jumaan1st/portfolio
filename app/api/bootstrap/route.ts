...
export async function GET(request: Request) {
    try {
        ...
        if (process.env.NODE_ENV === 'development') {
            data = await fetchBootstrapRaw(mode);
        } else {
            try {
                data = await getBootstrapData(mode);
            } catch (cacheError) {
                console.warn('Cache failed, fetching directly:', cacheError);
                data = await fetchBootstrapRaw(mode);
            }
        }

        // Ensure JSON parsing if necessary
